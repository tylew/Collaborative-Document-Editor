 ...
 * Format: [type=2][varuint: payload_len][payload]
 * payload: [varuint client_id][varuint json_len][json bytes]
 * Pass empty JSON to indicate removal.
 */
export function encodeAwareness(clientId: number, state: object | null): Uint8Array {
  const jsonStr = state ? JSON.stringify(state) : '';
  const jsonBytes = new TextEncoder().encode(jsonStr);

  const clientVar = encodeVarUint(clientId);
  const jsonLenVar = encodeVarUint(jsonBytes.length);

  const payloadLen = clientVar.length + jsonLenVar.length + jsonBytes.length;
  const payloadLenVar = encodeVarUint(payloadLen);

  const result = new Uint8Array(1 + payloadLenVar.length + payloadLen);
  let pos = 0;
  result[pos++] = MessageType.AWARENESS;
  result.set(payloadLenVar, pos);
  pos += payloadLenVar.length;
  result.set(clientVar, pos);
  pos += clientVar.length;
  result.set(jsonLenVar, pos);
  pos += jsonLenVar.length;
  if (jsonBytes.length > 0) {
    result.set(jsonBytes, pos);
  }
 ...
 * Returns { clientId, state (object|null) }
 */
export function decodeAwareness(data: Uint8Array): { clientId: number; state: any | null } {
  if (data.length < 2 || data[0] !== MessageType.AWARENESS) {
    throw new Error('Not an awareness message');
  }

  let pos = 1;
  const [payloadLen, lenBytes] = decodeVarUint(data.subarray(pos));
  pos += lenBytes;

  if (pos + payloadLen > data.length) {
    throw new Error('Incomplete awareness payload');
  }

  const payload = data.subarray(pos, pos + payloadLen);
  let p = 0;

  const [clientId, cidBytes] = decodeVarUint(payload.subarray(p));
  p += cidBytes;

  const [jsonLen, jsonLenBytes] = decodeVarUint(payload.subarray(p));
  p += jsonLenBytes;

  if (p + jsonLen > payload.length) {
    throw new Error('Incomplete awareness json');
  }

  let state: any | null = null;
  if (jsonLen > 0) {
    const jsonStr = new TextDecoder().decode(payload.subarray(p, p + jsonLen));
